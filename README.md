...
> **Статус:** React/Vite, `report.json` и localhost viewer включены в `main`. Кодовый срез `a086162` с объяснениями HA-17 и гипотезами кластеров HA-18 прошёл пять официальных Docker-выпусков, контрактные тесты, offline запуск и локальный Playwright. Медиана batch-запуска готового образа — 4,619 с, максимум — 5,656 с; точные условия и ограничения — в [протоколе](docs/validation.md). Дневные профили HA-11, запросы HA-14, датированные маршруты HA-16 и межкластерный обзор HA-13 в этот выпуск не входят.
