...
> **Статус:** React/Vite, `report.json` и localhost viewer включены в `main`. Объяснения HA-17 и гипотезы кластеров HA-18 прошли пять официальных Docker-выпусков; медиана batch-запуска готового образа — 4,619 с, максимум — 5,656 с. Обновлённый интерфейс с Cytoscape.js проверен на выпуске с 2 248 клиентами, 3 119 связями и 89 кластерами: контрактный тест и просмотр на десктопе и телефоне через Playwright прошли. Условия, ограничения и оставшиеся задачи — в [протоколе](docs/validation.md).
...
В отдельном терминале запустите `docker compose up viewer`, затем откройте `http://127.0.0.1:8000/`: отчёт откроется сразу. Точные результаты и проверенные SHA ведутся в [протоколе](docs/validation.md).

В отчёте начните с топ-20 или найдите клиента по полному ID. На широком экране список, граф и карточка видны рядом; на телефоне карточка идёт сразу после поиска, а ссылки ведут к связям и списку. Вкладка «Все 2 248» открывает полный список с поиском по части ID и страницами по 50 строк. «Почему в списке» и карточка показывают ведущие факторы приоритета, а совпавшие правила объяснены обычными фразами с числами и порогами; исходные формулы можно раскрыть для проверки. Список кластеров показывает состав группы и первого клиента для проверки, сохраняя исходную формулировку отдельным раскрытием. Граф показывает крупнейшие направленные связи клиента; кнопка раскрывает все связи, а отдельный режим открывает круговую карту полного графа. Полные рёбра доступны в таблице под графом, дополнительные фильтры и легенда раскрываются по запросу. Для быстрой проверки введите несуществующий ID: интерфейс должен сообщить, что клиент не найден, и очистить прежний выбор.
...
- Frontend — React + Vite + Cytoscape.js. Python остаётся batch CLI и выпускает JSON/CSV; БД, backend API, GPU и Node на машине, где запускают готовый Docker-образ, не нужны.
...
Затем откройте `http://127.0.0.1:8000/` (адрес `/report.html` тоже работает). Viewer раздаёт `out/` только для чтения и привязан к localhost. Откройте отчёт через HTTP: React загружает соседний `report.json`; режим `file://` для этого контракта не поддерживается.
...
Откройте `http://127.0.0.1:8000/` в браузере на той же машине. Linux clean-clone маршрут пока не проверен на Linux-хосте; результат не следует считать подтверждённым до такого прогона.
...
В отдельном терминале запустите `.venv/bin/python -m http.server 8000 --bind 127.0.0.1 --directory ./out`, затем откройте `http://127.0.0.1:8000/`.

Сборка frontend и установка Python-пакетов требуют интернета. После сборки контейнерный pipeline и localhost viewer проверены с `--network none`; Chrome на хосте открыл готовый отчёт с блокировкой внешних запросов. Условия этих проверок записаны в [протоколе](docs/validation.md).
...
- `top_nodes.csv` — 20 узлов на официальном наборе по числовому приоритету и детерминированному tie-break;
- `report.json` — полный набор узлов, рёбер, кластеров, параметров и доступных дополнительных признаков;
- `index.html`, `report.html` и `assets/` — заранее собранный локальный React UI; оба HTML-адреса открывают один отчёт;
...
`out/` игнорируется Git. Три обязательных CSV создаются командами запуска выше вместе с HTML/JSON-отчётом.

## Как исследовать клиента

1. Выберите клиента в топ-20 или введите его полный `gid` в поиске. Карточка показывает основную роль, факты её назначения, причины других совпавших правил и основания приоритета. Список равных приоритетов включает также клиентов вне топ-20.
2. На направленном графе переключайте окрестность выбранного клиента и полный граф. Фильтры позволяют выбрать кластер, seed и основную роль либо любое совпавшее правило; таблица рёбер показывает направление, сумму и число переводов.
3. Раскройте кластер: для группы со связями гипотеза указывает первого клиента для проверки по приоритету и наблюдаемые факты; для изолята — отсутствие наблюдаемых связей. Выгрузите `nodes_roles.csv`, `clusters.csv` и `top_nodes.csv` по ссылкам отчёта.

Для защиты есть [пятиминутный сценарий и репетиция объяснения трёх произвольных gid](docs/DEMO.md). Эти примеры не ограничивают поиск: отчёт содержит все 2 248 узлов.
...
[Независимая слепая техническая проверка 41 случая](docs/reviews/HA-12.1-blind-review-2026-09-23.md) не обнаружила расхождений числовых объяснений с наблюдаемыми рёбрами и порогами; в 11 случаях совпавших ролевых правил не было. Проверку выполнил агент без квалификации AML-аналитика; экспертная полезность и [время работы человека с тремя gid](docs/reviews/HA-12.1-timing-2026-09-23.md) пока не измерены.
...
Проект подаётся через командный GitHub-репозиторий, без отдельной формы. До дедлайна 18:00 по Астане нужно проверить опубликованный `main` и его SHA; команды финальной сверки приведены в [сценарии демо](docs/DEMO.md#подача-проекта).
