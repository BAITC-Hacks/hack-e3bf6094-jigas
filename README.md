...
> **Статус:** React/Vite, `report.json` и localhost viewer включены в `main`. Кодовый срез `a086162` прошёл пять официальных Docker-выпусков, контрактные тесты, offline запуск и локальный Playwright; точные результаты и ограничения — в [протоколе](docs/validation.md).
...
Сборка frontend и установка Python-пакетов требуют интернета. После установки приложение рассчитано на работу без внешней сети. Локальный Playwright smoke отчёта на localhost прошёл без внешних запросов; запуск с полностью отключённой сетью отдельно не подтверждён в [протоколе проверки](docs/validation.md).
...
При разработке и проверке этого репозитория подтверждённо использовался OpenAI Codex, включая субагентов GPT-6 Luna. Выводы об аналитических ролях и приоритетах проверяются кодом, контрактными тестами и протоколом выпуска; они не являются экспертной оценкой AML.
