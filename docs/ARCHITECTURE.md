...
Текущий общий контракт — PRODUCT_SPEC 1.3 / SPEC 1.4 / PLAN 1.3. HA-17 добавила объяснения в `hackalem/scoring.py`/report, а React показывает причины, общую шкалу рёбер, вторичные роли и равные P. HA-11, HA-14 и HA-16 добавили дневные профили, запросы продолжения и датированные маршруты как необязательные поля того же `report.json`; основная формула ролей/P и три CSV сохранены. HA-13 (SCC и межкластерные потоки) пока остаётся отдельной P1-задачей. Все новые экраны используют тот же viewer и выбранный `gid`.
