...
Контейнер `pipeline` собирается из `Dockerfile` через `compose.yaml`. Сервис `prepare` монтирует архив только для чтения и извлекает ровно три нужных файла через `scripts/prepare_data.py`. `pipeline` получает Parquet в `/app/data` только для чтения, результат пишет в `/app/out`. Образ не содержит исходный датасет; папки `data/` и `out/` исключены из Git и build context. Та же команда `starter.py --data --out` работает вне контейнера для самостоятельной проверки экспертами.
