...
Расчёт остаётся конечным Python CLI: `python starter.py --data ./data --out ./out` (также `python -m hackalem`). Модули `hackalem/` валидируют вход, вычисляют признаки/роли/приоритет/сообщества, SCC и направленные потоки между сообществами; из общего объекта выпускаются CSV и JSON. `gid` — точные целые в расчёте и строки в JSON/JS, деньги считаются в тиынах. React отображает вычисленные значения, Cytoscape.js рисует узловой и межкластерный графы; выбранный узел и фильтры хранятся в React. Позиции узлового графа задаются детерминированно, без браузерного пересчёта ролей или метрик.
...
Текущий общий контракт — PRODUCT_SPEC 1.3 / SPEC 1.4 / PLAN 1.3. HA-17 добавила объяснения в `hackalem/scoring.py`/report, а React показывает причины, общую шкалу рёбер, вторичные роли и равные P. HA-11, HA-14 и HA-16 добавили дневные профили, запросы продолжения и датированные маршруты как необязательные поля того же `report.json`. HA-13 добавила `community_edges`, `sccs`, `coverage` и узловые признаки: Python считает сводки, React показывает направленный мета-граф, исходные рёбра выбранного потока и отдельную подсветку SCC. Основная формула ролей/P и три CSV сохранены; P0-отчёт без новых полей остаётся читаемым.
