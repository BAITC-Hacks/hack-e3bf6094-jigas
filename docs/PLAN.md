...
- В этой редакции расширены HA-11/HA-12 и добавлены **HA-13/HA-14 с четырьмя дочерними задачами**: локальный план содержит 14 этапов и 28 подзадач (20 P0, 8 P1). Новые задачи описаны в §9; различия с ранее опубликованными карточками Trello — в §10.
...
Исходный контекст доски — [HA-CTX](https://trello.com/c/jhV5X3jU). Принятые изменения этой редакции фиксируются локально в спецификациях и §10; старое описание карточки не отменяет актуальный контракт.
...
Первоначальные HA-01–HA-12 сохранены как родительские карточки в «Этапы MVP», с двумя подзадачами и чек-листом у каждой. Сроки P0 были заданы на 23.09.2026 и наследуются детьми, P1 — без обещания выполнения в текущем спринте. Источник статусов созданных карточек — Trello; этот документ задаёт актуальный объём и зависимости. HA-13/14 пока представлены локальными задачами без карточек на доске.
...
| HA-13 · локальный этап | Структурный обзор: сообщества, SCC и покрытие — P1 |
| HA-14 · локальный этап | Следующий запрос данных и frontier-фильтр — P1 |
...
| HA-13.1 · P1 · локальная | Посчитать n_payer_comms, SCC, community_edges и coverage | HA-11.2 | Сверки §10.2 SPEC; суммы/направления/уникальные получатели; сохранить базовые CSV | 25 |
| HA-13.2 · P1 · локальная | Добавить обзор межкластерных потоков и подсветку SCC | HA-13.1 | Переходы к составу/исходным рёбрам; отдельные обозначения SCC/сообществ; offline | 25 |
| HA-14.1 · P1 · локальная | Рассчитать next_data_requests и boundary_gids_by_inflow | HA-13.2 | Каждая причина соответствует §10.3 SPEC; индекс из всех frontier; P и роли сохранены | 10 |
| HA-14.2 · P1 · локальная | Показать запросы в карточке и фильтр полной таблицы | HA-14.1 | Узлы вне top-20 доступны; сортировка входа не подменяет глобальный rank; поиск/возврат к топу | 15 |
...
## 10. Изменения для синхронизации с Trello

**Состояние этой редакции: обновлены локальные документы; описания и карточки Trello ещё не синхронизированы.** Ссылки §8–9 указывают на ранее созданные карточки, четыре новые подзадачи пока существуют в этом плане. Прежде чем брать старую карточку, читать актуальные SPEC/PRODUCT_SPEC и изменения ниже; статусы/исполнители на доске этим документом не переопределяются.

| Карточки | Подготовленное изменение |
|---|---|
| HA-00, HA-CTX | Сослаться на принятый merge, PRODUCT_SPEC 1.1/SPEC 1.2/PLAN 1.1 и аудит; указать обязательное ядро и последовательность P1 |
| HA-11, HA-11.1, HA-11.2 | Расширить дневной профиль прямыми seed-плательщиками и дневными unique counts; заменить стартовую зависимость HA-08.2 на HA-08.1; критерии — SPEC §10.1 и реестр §9 |
| HA-08.2 | Финальная проверка фактического состава релиза после всех принятых P1; она не является барьером старта P1 |
| HA-12, HA-12.1, HA-12.2 | Зафиксировать слепой review, контроль, ограничения метрик, чувствительность и отдельную оценку вариантов; не заявлять улучшение по одному overlap |
| HA-13, HA-13.1, HA-13.2 | Создать этап структурного обзора и две P1-задачи по §9 с зависимостью от HA-11.2 |
| HA-14, HA-14.1, HA-14.2 | Создать этап запросов данных и две P1-задачи по §9 с зависимостью от HA-13.2 |
| HA-09, HA-10 и их дети | При завершении отразить реализованный P1 в README/демо; общий сценарий по-прежнему 5 минут |

При синхронизации сохраняются существующие ID, исполнители, статусы и история; новые задачи получают P1 без выдуманных due dates. После создания карточек заменить локальные обозначения реальными ссылками и отметить факт синхронизации здесь. Принятые P1 не становятся выполненными по обновлению документов или доски.
