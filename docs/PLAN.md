...
|---|---|---|
| A — аналитика | `hackalem/validation.py`, `graph.py`, `scoring.py`, `clusters.py`, `report.py`, `exports.py`: HA-01.1 → HA-05.2 | Один активный владелец каждого модуля; зависимости между этапами передаются через согласованные функции, даже независимый математически Louvain идёт после скоринга |
| B — интерфейс | `report_template.html`, `report.example.json`, `assets/`, `hackalem/html.py`: HA-06.1 → HA-06.2 | Работает по JSON-контракту и не меняет аналитику |
| B — интеграция | После HA-05.2 принимает `hackalem/pipeline.py` и фасад `starter.py` для HA-07.1 | Исправления аналитики возвращает A; не меняет формулы на стыке; фасад сохраняет прежние публичные функции и CLI |
| A/B — дополнения | После HA-07.2 и HA-08.1 A ведёт признаки в `graph.py`, сводки в `clusters.py`, отчётные поля в `report.py` и правила HA-14.1 в новом `requests.py`; B ведёт UI-функции в `hackalem/html.py`, шаблоне и примере | Пары выполняются по порядку §4; `pipeline.py` и фасад интегрируются отдельным handoff; один владелец файла, C проверяет принятые дополнения до финальной HA-08.2 |
...
Точки изменения сверены с реальным starter в архиве и текущей модульной реализацией. В [SPEC §11](SPEC.md#11-точки-реализации-для-агентов) указаны целевые публичные сигнатуры, модули реализации, порядок таблиц, точные имена колонок, маркеры HTML и команда проверки. Корневой `starter.py` остаётся совместимым фасадом; production-изменения вносятся в соответствующие `hackalem/*.py`. Для HA-14.1 `hackalem/requests.py` создаётся как внутренний модуль, без новой внешней зависимости. Готовность каждой функции подтверждается отдельным handoff и приёмкой.
...
|---|---|---|
| HA-13.1 / A | Валидированный G, df с cluster_id; `hackalem/clusters.py`, `report.py` | Поля/сводки SPEC §10.2 в одном report. Сверить раздельные направления, internal + inter-community = total, SCC с unique external dst, frontier по nodes.depth[dst]. Передать фактическую сверку B/C. |
| HA-13.2 / B | Объект HA-13.1; `hackalem/html.py`, `report_template.html`, `report.example.json` | Обзор на существующем vis-network, клик по сообществу/стрелке раскрывает исходные узлы/рёбра, SCC — отдельная подсветка. Проверить singleton/no-SCC>1, точный gid и отсутствие блока на P0-примере. Передать готовый просмотр C и владельцу HA-14.1. |
| HA-14.1 / A | df и daily_profiles_by_gid; новый `hackalem/requests.py`, `report.py` | Список запросов с reason_code по условиям SPEC и отсортированный индекс frontier. Проверить boundary, короткий период, same-day и изолят; суммы/роли/priority совпадают с baseline. Передать B/C. |
| HA-14.2 / B | Объект HA-14.1; `hackalem/html.py`, `report_template.html`, `report.example.json` | Причина + запрос в существующей карточке; фильтр по полному индексу, исходный priority, ранг только из top_nodes. Проверить узел вне топа, пустой фильтр, изолят, отсутствующий P1-блок и клавиатуру. Передать C для финального выпуска. |
